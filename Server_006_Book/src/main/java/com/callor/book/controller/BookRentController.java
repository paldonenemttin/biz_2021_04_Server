	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (subPath.equals("/seq")) {
			...
			if (strSeq == null || strSeq.equals("")) {
				...
				// view에서 보여줄 데이터 생성
				/*
				 * ServletContext app Tomcat을 기반으로 작성된 Web APP Service에서 요청(Req) 응답(Res)를 총괄하는
				 * 정보가 담긴 객체
				 * 
				 * Web App Service를 구현하기 위하여 Req, Res 를 처리하는 여러가지 기능을 구현해야 하는데 그러한 기능을 미리 구현해
				 * 놓았기 때문에 SerlvetContext를 getter 하는 것만으로 충분하다
				 * 
				 * DB 등으로 부터 조회된 데이터를 Web 에게 응답하고자 할때 쉬운 방법으로 전달할수 있도록 하는 기능이 미리 구현되어 있다
				 */
			...
			if (bu_name == null || bu_name.equals("")) {
				...
				//req 객체에 바로 셋팅하기
				req.setAttribute("BUYER", buList);
				
				// page1.jsp 파일을 열고 BUYERS 변수와 함게 Rendering 을 하여 HTML 코드를 생성하라
