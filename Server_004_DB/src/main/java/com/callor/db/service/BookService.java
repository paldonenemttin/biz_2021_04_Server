...
public interface BookService {
	 ...
	 * Controller에서 Service의 select() method를 호출할 것이다
	 * 
	 * select() method는
	 * DB로 부터 데이터를 읽어서 Controller에게 return한다 
	 */
	...
	/*
	 * 변경하고자 하는 데이터를 전달 받아서
	 * (전달받은 데이터에는 반드시 PK값이 포함되어 있어야 한다)
	 * PK로 조건을 설정하고
	 * 데이터를 UPDATE
	 */
	public void update(BookVO bookVO); // UPDATE, 데이터 변환(갱신, 수정)
	
	// 삭제하고자 하는 데이터의 PK값을 매개변수로 전달받고
	// 1개의 데이터를 삭제한다
	public void delete(String bk_isbn); // DELECT, 데이터 삭제
